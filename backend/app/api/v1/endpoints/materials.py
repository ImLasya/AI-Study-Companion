...
Handles PDF upload, material status queries, and processing retries
...
async def upload_material(
    ...
    Validates the file and persists it to storage, creates a Material
    record with status 'queued', and schedules background processing.

    Returns HTTP 202 Accepted immediately while processing continues
    in the background.
    """
    service = MaterialService(db)
...
async def list_materials(
    ...
    """List all materials belonging to the specified project
...
async def get_material(
    ...
    """Fetch status, failure reason, and page count
...
async def retry_material(
    ...
    """Reset a failed material to 'queued' status and schedule
    background processing.
    """
    service = MaterialService(db)
    material = await service.retry_material(
        current_user.id,
        material_id,
    )
